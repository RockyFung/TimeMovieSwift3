...
class ADView: UIView {
    ...
    func showViewAnimation() -> Void {
        ...
        UIView.animate(withDuration: 0.7, animations: {
            weakSelf!.transform = CGAffineTransform(scaleX: 1.2, y: 1.2)
        }) { (finished) in
//            weakSelf!.transform = CGAffineTransform.identity // 撤销所有效果
            UIView.animate(withDuration: 0.2, animations: {
                weakSelf!.transform = CGAffineTransform(scaleX: 1.0, y: 1.0)
            })
        }
